...
| :--- | :---: | :--- |
| **Прицел игры (`.blk`)** | Перетащить в окно | Создает чистый `.json` для WTDraw. |
| **Файл конструктора (`.json`)** | Перетащить в окно | Создает файл `_packed.blk` с настройками для папки `UserSights`. |
